...
""")
        ...
        for i, line in enumerate(lines):
            if line.strip() == "## Recent Failures":
                new_entry = f"- [{commit_info['short_hash']} - {commit_info['message']}]({os.path.basename(failure_path)}) ❌ ({commit_info['date'][:10]})\n"
                lines.insert(i + 1, new_entry)
                break
...
## 🗂️ Navigation
- [📝 Changelogs](changelogs/README) - All commit changelogs
- [🔍 Code Reviews](code-reviews/README) - Automated code reviews
- [❌ Failed Builds](failed-builds/README) - Build failure analyses
